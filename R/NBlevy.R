...
"NBlevy" <- function(N,
  ...
  "NB.loglik" <- function(gamprob) {
    ...
    ll <- sum( lgamma(gamw + N) - lgamma(gamw) - lgamma(N+1) + gamw * log(p) + N*log(q) )
  ...
  "NB.loglik.conc" <- function(gam) {
    ...
    p <- gam/(gam + R)
    ##cat(gam, p, "\n")
    sum( lgamma(gamw + N) - lgamma(gamw) - lgamma(N+1) + gamw * log(p) + N*log(1-p) ) 
  }
