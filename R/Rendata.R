  ...
  if (missing(data)) {
    ...
    if ( is(x, "Rendata") && !is.null(x$MAXdata) ) {
      ...
      if (length(effDuration) !=  length(data))
        stop("in object 'x', elements 'MAXinfo' and 'MAXdata' do not have the same number of blocks")
      flag <- TRUE
    ...
    if (is.null(data)) {
      ...
      if (length(data) != length(effDuration))
        stop("when 'data' is a list, 'effDuration' must be a numeric vector of length length(data)")
      ...
      if (length(effDuration) != 1L)
        stop("when 'data' is a vector, 'effDuration' must be a numeric with length 1")
...
makeOTSdata <- function(x,
  ...
  if (missing(data)) {
    ...
    if (is.null(data)) {
      ...
      if ( (length(data) != length(effDuration)) || (length(data) != length(effDuration)) )
        stop("when 'data' is a list, 'effDuration' and 'threshold' must be a numeric vectors of length length(data)")
      
      r <- unlist(lapply(data, length))
      block <- factor(rep(1:length(data), times = r), levels = 1:length(data))
       flag <- TRUE
    } else {
      if ((length(effDuration) != 1) || (length(threshold) != 1))
        stop("when 'data' is a vector, 'effDuration' and 'threshold' must be numeric with length 1")
      r <- length(data)
  ...
  if (!flag) {
    ...
    names(effDuration) <- names(r) <-  names(threshold) <- paste("block", 1:length(effDuration))
  }
...
summary.Rendata <- function(object, ...) {
  ...
  if (!is.null(object$OTmissing)) {
    dur <- as.numeric(as.POSIXct(object$OTmissing$end) - as.POSIXct(object$OTmissing$start),
                      units = "days")
    ans$OTmissing <- sprintf("o missing 'OT' periods, total %5.2f years", sum(dur)/365.25)
