...
Renouv <- function(x,
  ...
  myTransFuns <- transFuns(trans.y = trans.y, distname.y = distname.y)
  ...
  ## not know in genral which parameters are scale parameters.
  ##===========================================================================
  ...
  nobs <- length(y.OT)
  if (MAX$flag) 
    nobs <- nobs + length(z.MAX) 
  if (OTS$flag)  
    nobs <- nobs + length(z.OTS) + sum(r.OTS == 0)  
  ...
  ##
  ##
  ## The 'MixExp2' distribution should be handled as a special distribution
  ## in the future?
  ##
  ##===========================================================================
  ...
  if (distname.y %in% specials) { 
    ...
    ## This was added in versions > 0.5-2 due to repeated problems with hessian
    if (numDeriv) {
      ## require(numDeriv)
  ...
  if ( hist.MAX || hist.OTS ) {
    ...
    if (hist.MAX) {   ## homogenise with other obs.
      ...
      if (trace) {
        cat("\n")
        cat("Take into account MAX historical data of time-length",
            sum(w.MAX), "units\n")
      }
    }
    ...
    if (hist.OTS) {   ## homogenise with other obs. 
      ...
      if (trace) {
        cat("\n")
        cat("Take into account OTS historical data on time-length",
            sum(w.OTS), "units \n")
      }
    }
    ...
    if (force.start.H) {
      ...
      if ( is.na(loglikC(par.ini.y)) || !is.finite(loglikC(par.ini.y)) ) {
        ...
        if ( (distname.y == "gpd" && par.ini.y["shape"] < 0) ||
            (distname.y == "maxlo") ) {

          ## if one of the historical levels  is over the quantile with
          ## prob. 0.995 of the distribution, try to give a larger value to
          ## the scale parameter. 
          warning("initial parameters for the GPD distribution ",
    ...
    if (trace) {
      cat("o Initial values for 2-nd stage optimization\n")
      print(par.ini.y)
    }
    ...
    ##                     control = control.H)
    ##     } else {
    ##       opt <- try(optim(par = par.ini.y,
    ##                        fn = loglikC,
    ##                        method =  "BFGS",            
    ##                        hessian = !numDeriv,
    ##                        control = control.H))
    ##       if (inherits(opt, "try-error") ||
    ##           any(opt$par < parLower.y) ||
    ##           any(opt$par > parUpper.y)) { 
    ##         if (trace) {
    ##           cat("Unconstrained optim failed.\n")
    ##           cat("trying a constrained optim\n")
    ##         }
    ##         opt <- optim(par = par.ini.y,
    ##                      fn = loglikC,
    ##                      method =  "L-BFGS-B",
    ...
    if (numDeriv) {
      ...
      stop("you must use 'numDeriv = TRUE' since concentrated log-likelihood is used")
    ...
    ##=============================================================
    ## No historical data. Just rearrange results
    ##=============================================================
  ...
  ##======================================================================
  ## Perform Bartlett's (or Moran's) test for exponentiality
  ##======================================================================
